#ifndef TMCOMBI_RTREE_H
#define TMCOMBI_RTREE_H
...
///
    ...
    public:
        ...
        public:
            ...
            void GetBounds(ELEMTYPE * const a_min, ELEMTYPE * const a_max, unsigned int dim) {
                        ASSERT(IsNotNull());
                StackElement &curTos = m_stack[m_tos - 1];
...
// Users will likely replace its usage with a Stream implementation from their favorite API.
    ...
    RTREE_QUAL::RTree(unsigned int dim) : dim_(dim) {
        ...
        const std::vector<double> UNIT_SPHERE_VOLUMES = {
    ...
    RTREE_TEMPLATE
    void RTREE_QUAL::Insert(const ELEMTYPE * const a_min, const ELEMTYPE * const a_max, const DATATYPE &a_dataId) {
#ifdef _DEBUG
    ...
    RTREE_TEMPLATE
    void RTREE_QUAL::Remove(const ELEMTYPE * const a_min, const ELEMTYPE * const a_max, const DATATYPE &a_dataId) {
#ifdef _DEBUG
        for(int index=0; index < dim_; ++index)
        {
          ASSERT(a_min[index] <= a_max[index]);
    ...
    RTREE_TEMPLATE
    int RTREE_QUAL::Search(const ELEMTYPE * const a_min, const ELEMTYPE * const a_max,
                          std::function<bool(const DATATYPE &)> callback) const {
#ifdef _DEBUG
        for(int index=0; index < dim_; ++index)
        {
    ...
    bool RTREE_QUAL::SaveRec(Node *a_node, RTFileStream &a_stream) {
        ...
        if (a_node->IsInternalNode())  // not a leaf node
        {
            for (int index = 0; index < a_node->m_count; ++index) {
                ...
                a_stream.WriteArray(curBranch->m_rect.m_min, dim_);
                a_stream.WriteArray(curBranch->m_rect.m_max, dim_);
...
// The exact volume of the bounding sphere for the given Rect
    ...
    ELEMTYPEREAL RTREE_QUAL::RectSphericalVolume(Rect *a_rect) {
        ...
        if (dim_ == 3) {
            ...
            return (ELEMTYPEREAL) (pow(radius, dim_) * m_unitSphereVolume);
        }
    }
