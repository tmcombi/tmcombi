#define BOOST_TEST_MODULE test_fs_auc_adult_transformed_dataset
#include <boost/test/included/unit_test.hpp>

//#define TIMERS
//#define TRACE_EVALUATOR

#include "../lib/classifier_creator_fs_auc.h"
#include "../lib/classifier_creator_train_tmc.h"
#include "../lib/evaluator.h"


BOOST_AUTO_TEST_CASE( test_fs_auc_adult_transformed_dataset ) {
    ...
    std::shared_ptr<ClassifierCreatorFsAuc> pCCFS = std::make_shared<ClassifierCreatorFsAuc>();
    pCCFS->verbose(true);
    ...
    BOOST_CHECK_EQUAL(roc_err_train, 0.15693840413810234);
    BOOST_CHECK_EQUAL(roc_err_eval, 0.15693658793853266);
    std::pair<std::pair<double, double>, std::pair<double, double>> confusion_matrix_train_({{392247103,23720282},{1020382813,4288211421}});
    BOOST_CHECK(confusion_matrix_train == confusion_matrix_train_);
    std::pair<std::pair<double, double>, std::pair<double, double>> confusion_matrix_eval_({{188719871,11891187},{510214631,2144796847}});
