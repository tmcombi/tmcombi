...
## nginx
docker pull nginx
docker run -d -v nginx_html:/usr/share/nginx/html:ro -v nginx_conf:/etc/nginx:ro -p 80:80 -p 443:443 --restart unless-stopped --net=testServerNetwork nginx

## jenkins
docker pull jenkins/jenkins:lts
docker run -d -v jenkins_home:/var/jenkins_home --restart unless-stopped --env JENKINS_OPTS="--prefix=/jenkins" --env 'JAVA_OPTS=-Dhudson.model.DirectoryBrowserSupport.CSP="sandbox allow-scripts;"'  --net=testServerNetwork  --network-alias jenkins jenkins/jenkins:lts
