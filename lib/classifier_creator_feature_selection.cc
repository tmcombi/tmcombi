...
BOOST_AUTO_TEST_CASE( classifier_creator_feature_selection_basics_weights )
{
    ...
    p = {10,2}; conf = {1.0,1.0}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    ...
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), conf.first);
    p = {7,10}; conf = {0.42857142857142855,1}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), 0.70833333333333337);
    p = {11,10}; conf = {0,0.21621621621621623}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), 0.14545454545454545);
    p = {4,6}; conf = {1,1}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), conf.second);
    p = {7,13}; conf = {0,0.42857142857142855}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), 0.2289156626506024);
    p = {14.5,0.5}; conf = {0.53846153846153844,1}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), 0.75609756097560976);
    p = {-1,16}; conf = {0,1}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), (10+8+18+45)/(double)(50+10+16+8+9+18+9+45));
    p = {-1,18}; conf = {0,1}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), (10+8+18+45)/(double)(50+10+16+8+9+18+9+45));
    p = {12.5,1.5}; conf = {1,1}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), 1);
    p = {4,8}; conf = {1,1}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), 1);
    p = {3,13}; conf = {0.59259259259259256,0.59259259259259256}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), 0.59259259259259256);
    p = {1,18};  conf = {0,1}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), (10+8+18+45)/(double)(50+10+16+8+9+18+9+45));
    p = {15,5.5}; conf = {0.35714285714285715,0.53846153846153844}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), 0.43902439024390244);
    p = {18,1}; conf = {0,0.53846153846153844}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), 0.27083333333333331);
    p = {8,12}; conf = {0.42857142857142855,0.42857142857142855}; BOOST_CHECK(pFsClTmc->confidence_interval(p) == conf);
    BOOST_CHECK_EQUAL(pFsClTmc->confidence(p), 0.42857142857142855);
}
