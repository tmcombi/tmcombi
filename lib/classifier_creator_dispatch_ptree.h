...
#include "feature_transform_creator_dispatch_ptree.h"
...
ClassifierCreatorDispatchPtree::ClassifierCreatorDispatchPtree(const boost::property_tree::ptree & pt) : pC_(nullptr) {
    ...
    if ( type == "ClassifierTmc" ) {
        ...
        const auto pFTC = std::make_shared<FeatureTransformCreatorDispatchPtree>( pt.get_child("feature_transform") );
        pC_ = std::make_shared<ClassifierTransformedFeatures>(pCC->get_classifier(),pFTC->get_feature_transform());
    } else {
